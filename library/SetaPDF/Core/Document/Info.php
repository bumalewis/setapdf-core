 ...
 * @version    $Id: Info.php 816 2016-02-12 08:50:35Z jan.slabon $
 */
...
class SetaPDF_Core_Document_Info
{
    ...
    public function setTitle($title, $encoding = 'UTF-8')
    {
        ...
        if ($this->getSyncMetadata() !== false) {
            $this->updateXmp(
                ...
                'title',
                $title ? SetaPDF_Core_Encoding::convert($title, $encoding, 'UTF-8') : $title
            );
        }
    ...
    public function setAuthor($author, $encoding = 'UTF-8')
    {
        ...
        if ($this->getSyncMetadata() !== false) {
            ...
            if ($author) {
                $author = SetaPDF_Core_Encoding::convert($author, $encoding, 'UTF-8');
                $author = $this->_extractParts($author);
            } else {
                $author = false;
            }
    ...
    public function setSubject($subject, $encoding = 'UTF-8')
    {
        ...
        if ($this->getSyncMetadata() !== false) {
            $this->updateXmp(
                ...
                $subject ? SetaPDF_Core_Encoding::convert($subject, $encoding, 'UTF-8') : $subject
            );
        }
    ...
    public function setKeywords($keywords, $encoding = 'UTF-8')
    {
        ...
        if ($this->getSyncMetadata() !== false) {
            if ($keywords) {
                $keywords = SetaPDF_Core_Encoding::convert($keywords, $encoding, 'UTF-8');
                $keywords = $this->_extractParts($keywords);
            } else {
                $keywords = false;
            }
    ...
    public function setCreator($creator, $encoding = 'UTF-8')
    {
        ...
        if ($this->getSyncMetadata() !== false) {
            $this->updateXmp(
                ...
                'CreatorTool',
                $creator ? SetaPDF_Core_Encoding::convert($creator, $encoding, 'UTF-8') : $creator
            );
        }
    ...
    public function setProducer($producer, $encoding = 'UTF-8')
    {
        ...
        if ($this->getSyncMetadata() !== false) {
            $this->updateXmp(
                'http://ns.adobe.com/pdf/1.3/',
                'Producer',
                $producer ? SetaPDF_Core_Encoding::convert($producer, $encoding, 'UTF-8') : $producer
            );
        }
    ...
    public function setCreationDate($date)
    {
        ...
        if ($date === null) {
            ...
            if ($this->getSyncMetadata() !== false) {
                $this->updateXmp('http://ns.adobe.com/xap/1.0/', 'CreateDate', false);
            }
            ...
            if ($this->getSyncMetadata() !== false) {
                $this->updateXmp('http://ns.adobe.com/xap/1.0/', 'CreateDate', $date->getAsDateTime()->format('c'));
            }
        }
    ...
    public function setModDate($date)
    {
        ...
        if ($date === null) {
            ...
            if ($this->getSyncMetadata() !== false) {
                $this->updateXmp('http://ns.adobe.com/xap/1.0/', 'ModifyDate', false);
            }
            ...
            if ($this->getSyncMetadata() !== false) {
                $this->updateXmp('http://ns.adobe.com/xap/1.0/', 'ModifyDate', $date->getAsDateTime()->format('c'));
            }
        }
    ...
    public function setCustomMetadata($name, $value, $encoding = 'UTF-8')
    {
        ...
        if ($this->getSyncMetadata() !== false) {
            $this->updateXmp('http://ns.adobe.com/pdfx/1.3/', $name, $value);
        }
    ...
    {
        if ($this->getSyncMetadata() === false) {
            throw new BadMethodCallException('You have to call setSyncMetadata(true) prior calling this method.');
        }
    ...
    public function updateXmp($namespace, $tagName, $value)
    {
        ...
        if ($elements->length === 0) {
            ...
            $description = $this->_findDescription($namespace, true);
    ...
    private function _escapeTagChar($matches)
    {
        ...
        $char = "\xE2\x86\x82"
            . SetaPDF_Core_Type_HexString::str2hex(
                SetaPDF_Core_Encoding::convert($char, 'UTF-8', 'UTF-16BE', $char)
            );
    ...
    {
        $tagName = preg_replace_callback('/[^\pL\pN\.\-_]/u', array($this, '_escapeTagChar'), $tagName);

        return preg_replace_callback('/^[^\pL_ↂ]/u', array($this, '_escapeTagChar'), $tagName);
    }
